...
/**
 * The named product owner whose approval Stage 0 requires.
 *
 * Upstream pins its founder's personal name here. This fork carries no
 * personal attribution in shipped code (owner-branding-removal), so the
 * identity is a fork-owned constant instead. The check keeps its full meaning
 * — approval must come from this exact named owner, and any other name still
 * yields `named_product_owner_approval_missing` — while the two fields stay
 * pinned to exact literals, which is what LITERAL_PINNED_KEYS above relies on.
 */
export const PRODUCT_OWNER_APPROVER_NAME = 'WorldMonitor Product Owner';
export const PRODUCT_OWNER_APPROVER_ROLE = 'WorldMonitor product owner';
...
export function evaluateStage0(
  ...
  if (
    approval.approverName !== PRODUCT_OWNER_APPROVER_NAME
    || approval.approverRole !== PRODUCT_OWNER_APPROVER_ROLE
  ) {
