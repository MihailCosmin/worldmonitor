...
### [x] D15. Correct Pro marketing source and rebuild static output
...
- Resolution (2026-07-27):
  - Verified what remains genuinely paid by reading `src/shared/premium-paths.ts` (the single server+client gating source of truth) plus every live `hasPremiumAccess`/`isProUser`/`hasTier` call site: scheduled digest + alert-rule delivery to Slack/Discord/Telegram/Email/webhook (`hasTier(1)` gate, server 403s free callers), MCP access (`/api/mcp-proxy`, `mcp/auth.ts`'s `pro`/`user_key` auth kinds — no anonymous path exists), priority near-real-time data refresh (confirmed in D14's `gateway-pro-fresh-cache` test), and the Pro tier of the Widget Builder (smarter model, more turns, larger builds, interactive JS — base builder is free). Confirmed stale: WM Analyst chat, stock analysis/backtesting, Scenario Engine, Route Explorer, AI Market Implications, Regional Intelligence, analytical framework selection, all 56 map layers, and flight search — all now free with zero gating code anywhere.
  - Rewrote `pro-test/src/locales/en.json` (root source), `convex/config/productCatalog.ts`'s `marketingFeatures` (the true source `scripts/generate-product-config.mjs` derives `tiers.json` and syncs `en.json` from — editing only `en.json` would have silently reverted on next regen), `index.html`, `welcome.html`, `prerender.mjs`'s hardcoded prose (fixed a same-file self-contradiction where it told crawlers the Scenario Engine/Route Explorer/MCP were already free, then said Pro "adds" them one section later), `App.tsx` (trimmed a hardcoded 9-item Pro feature array to the 6 items that are actually still paid — padding back to 9 would have meant reintroducing stale claims or hollow restatements), and `Moments.tsx` (3 signals wrongly tagged "pro" — market data, energy assets, and weather/fires are all free).
  - Translated the same ~40 changed keys into all 24 non-English locales (script-applied, not hand-edited per file, to stay consistent and avoid transcription drift). Discovered the entire `welcome.*` namespace (used by the free homepage's pricing teaser/FAQ) was missing from 23 of 24 locales and correctly falling back to English via i18next — left that alone (out of scope, pre-existing) and removed `fa.json`'s copy of that namespace, which was an untranslated verbatim copy of the old English text, for consistency with the other 23.
  - Fixed 2 blog posts (stale "custom widgets are paid" claims, "scenario simulation, route risk" listed as paid) and 2 stale bullets in the `register-interest.ts` lifecycle email ("AI Analyst"/morning-briefs hook, "30+ services, 1 key" data-aggregation claim that the free tier makes identically) — left the waitlist/referral framing itself alone as an intentional, unrelated growth mechanic.
  - Rebuilt `public/pro` via `cd pro-test && npm run build` (not hand-edited). The rebuild caught a live regression suite (`tests/pricing-docs-drift.test.mjs`, `tests/product-catalog-freshness.test.mjs`) that enforces exact byte-for-byte parity between `productCatalog.ts` and: the JSON-LD `Offer.description` on `pro-test/index.html` (must be the literal joined `marketingFeatures`, not paraphrased prose — caught and fixed), and two independent hardcoded catalog mirrors (`scripts/ais-relay.cjs`, `api/product-catalog.js`) that a Redis-seeding path can serve in place of the generated fallback — both still had the fully-stale Free/Pro feature arrays and are now fixed. Also caught a Farsi-digit convention mismatch (`fa.json` used Persian-script "۵۰" where the cross-locale test and every other locale use Latin "50"). All 47 tests in those 3 files pass; `tsc --noEmit` and `biome lint` clean.

### [x] D16. Update documentation and generated API descriptions
...
- Resolution (2026-07-27):
  - Verified `src/config/panels.ts` has zero panels setting `premium: 'locked'`/`'enhanced'` and `apiKeyPanels` is now `[]` — confirmed every "PRO (hard-locked)"/"PRO-gated"/desktop-`_desktop &&`-gated claim across 13 panel/methodology docs was dead-code description, not current behavior. Fixed all 13: `chat-analyst`, `daily-market-brief`, `deduction`, `forecast`, `oref-sirens`, `sanctions-pressure`, `stock-analysis`, `stock-backtest` (also dropped stale "Premium " title prefixes — components already render plain "Stock Analysis"/"Backtesting"), `strategic-posture`, `supply-chain`, `telegram-intel`, and methodology `chokepoints.mdx`/`country-resilience-index.mdx`.
  - Rewrote `docs/authentication.mdx`: the doc's entire "How to Configure Which Panels Are Premium" mechanism (`WEB_PREMIUM_PANELS`, `PanelGateReason`) no longer exists in source at all — replaced with the real remaining mechanism (`PREMIUM_RPC_PATHS` endpoint allowlist) and its actual 5 current entries. Caught and corrected my own draft claiming scheduled-digest/alert-rule delivery goes through `PREMIUM_RPC_PATHS` — it's actually a separate standalone check (`api/notification-channels.ts`'s own `entitlements.features.tier < 1` gate), fixed before finalizing.
  - Rewrote `docs/premium-finance.mdx` and `docs/premium-finance-search.mdx` (retitled away from "Premium Finance" framing; Stock Analysis/Backtesting/Daily Market Brief are free) while preserving the accurate architectural content (Redis storage model, request flow, data sources) and correctly keeping the real `premium-stock-store.ts` filename as-is (renaming it was out of scope).
  - Fixed `docs/features.mdx`, `docs/pricing.mdx`, `public/pricing.md` (including its machine-readable JSON feature arrays — verified the drift-guard test only checks numeric price fields, not feature text, so this needed a manual pass same as the pro-test site in D15), and `docs/DEPLOYMENT-PLAN.md`'s QA checklist. Also fixed the same stale lines in `docs/zh/features.mdx`, `docs/zh/pricing.mdx`, `docs/zh/finance-data.mdx` where directly encountered (no full docs/zh/* audit — out of scope, unlike D15's explicit multi-language mandate).
  - Fixed 4 stale RPC comments in `market/v1/service.proto` (analyze-stock, get-stock-analysis-history, backtest-stock, list-stored-stock-backtests) and 3 in `supply_chain/v1/service.proto` (get-country-cost-shock, get-route-explorer-lane, get-route-impact — the latter two needed care: they wrap the genuinely-still-gated `route-intelligence` vendor endpoint's *compute function* directly rather than proxying its gated HTTP route, confirmed via the handler's own doc comment and an empty grep for any auth/403 logic). Fixed the matching stale comments in `server/gateway.ts` (regional-intelligence RPC_CACHE_TIER block) and `get-regional-snapshot.ts`'s docstring.
  - Installed Go 1.23 + `buf` v1.64.0 + the 3 sebuf protoc plugins user-locally (tarball/`go install`, no sudo available or needed) and ran `make generate`. This surfaced two real issues beyond the stale-claims scope: a proto-comment reference in `prerender.mjs` that would have rendered literal `undefined` (caught by the prerender script's own guard — from D15, already fixed before this task), and a pre-existing, unrelated latent bug where `resilience/v1/service.proto`'s `GetResilienceScore`/`GetResilienceRanking` RPCs had no description at all (hidden until someone actually ran the full generator) — fixed with accurate descriptions since leaving it would have shipped a newly-failing test.
  - All 588 tests across the OpenAPI contract suite, `route-cache-tier.test.mjs`, and `pricing-docs-drift.test.mjs` pass; `tsc`/biome clean.

### [x] D17. Perform a source-aware final `Pro` / `Premium` allowlist sweep
...
- Resolution (2026-07-28):
  - Established the complete, code-verified gating truth before classifying anything: `PREMIUM_RPC_PATHS` (5 entries), `ENDPOINT_ENTITLEMENTS` (2 entries, both also in the first list), plus two standalone checks outside either registry — `api/notification-channels.ts` (scheduled digest/alert-rule delivery) and `api/widget-agent.ts` (AI Widget Builder). That's the entire hard-gated surface; everything else is free regardless of what a comment or doc claims.
  - Fixed 4 public agent-skills files that falsely claimed entitlement-tier gating with no code backing it: `check-sanctions-pressure`, `track-tariff-trends`, `trace-trade-flows` (removed the stale `**Entitlement:** ... Pro-gated ... 403` line + matching Errors-section row), and `fetch-resilience-score` (larger rewrite — it falsely required auth outright, not just Pro tier; the proto already says `GetResilienceScore` is "Free for all users"). Fixed the same 3 stale `(Pro-gated)` annotations at their actual source, `scripts/build-agent-skills-index.mjs` (not just the generated `index.json`), and regenerated.
  - Fixed `proto/worldmonitor/economic/v1/service.proto`'s `ListGlobalTenders` comment and `docs/mcp-tools-reference.mdx`'s matching procurement-tool description, then ran `make generate` (toolchain still installed from D16) to sync `docs/api/EconomicService.openapi.*` and the bundle.
  - Found a 5th stale claim only the test suite caught: `public/.well-known/mcp/server-card.json` had a hand-frozen `get_procurement_opportunities` description reading "canonical Pro route" while the live `api/mcp/registry` (the actual source of truth) already said "canonical paginated route" — `tests/mcp-tools-list-compression.test.mjs` polices this drift explicitly. Regenerated the one field from the registry per the test's own instructions.
  - Corrected my own initial misclassification: I first filed AI Widget Builder as a "free baseline, Pro gets a smarter model" quota tier, based on marketing copy and the frontend component having no visible gate. Reading the actual backend (`api/widget-agent.ts`) disproved this — a signed-in free (tier 0) user gets a straight `403 Pro subscription required` with no fallback; it's a genuine hard gate, same class as notification-channels. Corrected before writing the allowlist doc.
  - Verified and left unchanged as legitimate (not stale): `api/api-route-exceptions.json`'s `mcp-grant-context.ts` note (real OAuth grant flow for the already-gated `/api/mcp-proxy`, unrelated to data access); `convex/broadcast/proLaunchEmailContent.ts` (both claimed Pro perks — Widget Builder, MCP — are genuinely still Pro-only or Pro-enhanced, confirmed against `api/widget-agent.ts` and the MCP OAuth grant flow); `docs/api-keys.mdx` / `UnifiedSettings.ts`'s API-key upgrade gate (manual `wm_` key creation genuinely requires API Starter/Business — confirmed this doesn't make the SKILL.md fixes moot, since a free signed-in dashboard user connecting via OAuth/Bearer still benefits from the corrected tier claim); `SECURITY.md`, `shipping/v2` and `forecast/v1` proto comments, `docs/panels/forecast.mdx`, `docs/panels/news-feeds.mdx`, `docs/usage-auth.mdx`, `docs/api-notifications.mdx`, `tests/mcp-api-parity.test.mjs`, `api/internal/brief-why-matters.ts`, and blog posts — all either describe a real remaining gate or correctly state something is *not* gated.
  - A second, broader high-signal phrase sweep across the full D17-specified directory scope (not just the exact stale markers already fixed) turned up 3 more real drifts: `server/worldmonitor/supply-chain/v1/_insurance-tier.ts`'s "PRO-only" comment contradicted D16's own fix marking `get-country-cost-shock` free (zero auth in the handler); `docs/data-sources.mdx`'s "Detailed bilateral trade data remains Pro-only" claim about the China deep-dive panel was the same already-free UN Comtrade data under another name (fixed alongside its pinning assertion in `tests/china-coverage-production-registration.test.mts`); `docs/architecture/pro-monetization.md` (a living, git-tracked architecture doc, "Last verified 2026-07-06") still described "locked panels unlock" when zero panels have set `premium: true` since D13.
  - Found and fixed an unrelated pre-existing test failure while verifying: `pro-test/src/locales/fa.json` carried 4 stray keys (`apiSection.cli`, `tiers.priceMonthly`/`priceAnnual`/`annualSavingsNote`) that no other locale has and no current code path reads per-locale (i18next `fallbackLng` covers the first; `prerender.mjs` hardcodes `en.tiers.*` regardless of locale for the other three) — removed to match all 23 other locales and unblock `tests/pro-locale-registry.test.mjs`.
  - Wrote `docs/architecture/paid-capability-allowlist.md` — the exhaustive, code-verified list of every genuinely-still-paid capability, with an explicit "add here in the same PR" instruction for future gating changes. Deliberately placed under `docs/architecture/` (tracked, sibling to `pro-monetization.md`) rather than `docs/internal/`, which is entirely `.gitignore`d and would have made the allowlist invisible to any future audit. Backed it with `tests/pro-gated-allowlist.test.mjs`, which mechanically pins `PREMIUM_RPC_PATHS` and (via source-parsing, since it isn't exported) `ENDPOINT_ENTITLEMENTS` to the documented set and fails on any unreviewed addition or removal — the two standalone checks aren't registry-based and stay hand-synced against the doc.
  - Full `test:data` suite (16,943 tests) passes clean except 6 failures confirmed pre-existing and unrelated via a `git stash` A/B comparison (`readBootstrapTierObject`, dashboard critical-CSS graph, Dockerfile transitive-import closure, notification-channels relay-timeout recovery [flaky], data-loader oref wiring, panel-layout dynamic-import guard) — none touch Pro/Premium/pricing/locale surfaces. `tsc --noEmit` and `biome lint` clean on every touched file.
