...
| `WM_SESSION_SECRET` | Signs the anonymous browser session cookie (`api/wm-session.js`). Without it every visitor — including a signed-in local user, since this cookie is separate from your login — sees "Anonymous data is temporarily unavailable" and public panels stop loading. The app container refuses to start without it. | `openssl rand -hex 32` |
