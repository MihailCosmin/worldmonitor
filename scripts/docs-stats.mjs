...
function claims(s) {
  return [
    ...
    { file: 'public/pricing.md', re: /Includes: (\d+)\s+map layers/, value: s.layerDefinitions },
    ...
    { file: 'docs/pricing.mdx', re: /\*\*Free\*\* — (\d+)\s+map layers/, value: s.layerDefinitions },
    { file: 'docs/pricing.mdx', re: /(\d+)\+\s+observed upstream hosts/, value: s.sourceAttributionHosts },
    { file: 'docs/accounts.mdx', re: /(\d+)\s+map layers/, value: s.layerDefinitions },
...
/**
 * Upstream's #5387 guard asserts the inverse of what this fork needs.
 *
 * Upstream web-locks `resilienceScore`, so its plan copy MUST name Resilience
 * as the Pro-only layer. This fork ungated the map layers: LAYER_REGISTRY
 * web-locks nothing, all 56 layers are free, and any copy carving Resilience
 * out of the free tier is false advertising here. So the check is inverted —
 * the registry must stay unlocked, and no plan surface may name a Pro-only
 * layer.
 *
 * It is inverted rather than deleted because it still guards a real
 * regression in both directions: a sync that re-locks a layer, or one that
 * merges upstream's "all free except Resilience" copy back in, fails here.
 */
export function validatePlanLayerEntitlementCopy(stats, readFile = read) {
  ...
  if (locked.length > 0) {
    failures.push(
      `src/config/map-layer-definitions.ts: this fork serves every map layer free, but LAYER_REGISTRY now web-locks [${locked.join(', ')}] — a merge re-introduced map-layer gating`,
    );
    return failures;
  }

  const PRO_ONLY_CLAIM = /all free except Resilience|Resilience is Pro|except the Resilience layer|Pro-only Resilience|map layers except Resilience|Resilience 图层属于 Pro/;
  for (const file of PLAN_LAYER_COPY_SURFACES) {
    ...
    if (PRO_ONLY_CLAIM.test(text)) {
      failures.push(
        `${file}: plan copy carves the "${PLAN_LAYER_PRO_ONLY_LABEL}" layer out of the free tier, but this fork locks no map layers (#5387, inverted for this fork)`,
      );
    }
  }
  return failures;
