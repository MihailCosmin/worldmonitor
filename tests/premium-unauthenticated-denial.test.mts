...
/**
 * This fork's `api/chat-analyst.ts` predates this sync unchanged: it uses
 * `resolveAuthenticatedCallerIdentity` (any authenticated caller, any tier)
 * rather than upstream's Pro-gated `resolvePremiumCallerIdentity` classifier,
 * so it already answers a missing credential with a plain 401 — just with
 * this fork's own `{ error: 'Authentication required' }` body (the same
 * wording used across gateway.ts/entitlement-check.ts/mcp/auth.ts) rather
 * than upstream's `UNAUTHENTICATED` code. `classifyPremiumDenial` keys the
 * `sign_in_required` verdict on the 401 status alone, so the client-side
 * classification #5619 fixed is reachable either way — the last test below
 * pins that end to end. There is no confirmed-free 403 upsell to test here:
 * a signed-in free caller is not gated at all on this route (quota-limited,
 * not tier-limited), so it is out of scope for this file.
 */
describe('api/chat-analyst answers a missing credential with 401 (#5619)', () => {
  it('a signed-out caller is told to sign in, not to subscribe', async () => {
    ...
    assert.deepEqual(await res.clone().json(), { error: 'Authentication required' });
