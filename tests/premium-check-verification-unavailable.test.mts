...
/**
 * Upstream's `api/chat-analyst.ts` is Pro-gated and adopts the marker/billing
 * classification above to turn each billing state into a distinct status
 * (503 retryable, 403 upsell variants). This fork's Chat Analyst is free for
 * every authenticated caller — access is quota-limited
 * (`server/_shared/direct-llm-quota.ts`), not tier-limited — so the route
 * uses the simpler `resolveAuthenticatedCallerIdentity` wrapper and never
 * reaches billing classification at all. That predates this sync unchanged;
 * see `tests/chat-analyst.test.mts` for its actual auth/quota contract.
 */
describe('api/chat-analyst does not adopt tier-based billing classification', () => {
  it('does not import or reference the billing-denial classification surface', async () => {
    const source = await readFile(new URL('../api/chat-analyst.ts', import.meta.url), 'utf8');
    assert.doesNotMatch(
      source,
      /resolvePremiumCallerIdentity|billingDenial|X-Billing-Verification/,
      'Chat Analyst is free for every authenticated caller; it must not grow a Pro-tier billing gate',
    );
  });
});
