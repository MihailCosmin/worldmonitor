...
describe('premium panels route their denials through the classifier', () => {
  ...
  it('reports the only client-side entitlement-desync decision sites', () => {
    // Upstream lists three. LatestBriefPanel is not one of them on this fork:
    // api/latest-brief.ts's pro_required 403 was removed (see
    // evaluated_upstream_commits.exclude in the fork-sync manifest), so the
    // panel has no entitlement verdict to classify and its 401/403 handling
    // stays a plain sign-in / access-denied split. There is no desync to
    // report when nothing is gated.
