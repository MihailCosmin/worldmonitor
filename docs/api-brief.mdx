...
<Info>
`GET /api/latest-brief` requires a valid Clerk session but no PRO tier. Share creation (`/api/brief/share-url`) still requires PRO, and the public share route (`/api/brief/public/{hash}`) remains unauthenticated.
</Info>
...
Returns a short summary of the caller's most recent composed brief, or falls
back to the shared dashboard edition when the caller has no personal brief
yet. If neither the requested slot nor the current readable slot has produced
a brief, the route returns `{ status: "composing" }`.
...
slot. The `magazineUrl` is freshly signed against `{userId, issueSlot}` for
whichever edition the route resolved, so it only works for that brief owner.
