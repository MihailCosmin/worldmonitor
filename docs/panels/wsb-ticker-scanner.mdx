...
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** and **finance** variants, with `priority: 75` (surfacing lower in default panel grids than priority-1 panels). Not present in the tech, commodity, or happy variants. Source: `'wsb-ticker-scanner'` in `FULL_PANELS` and `FINANCE_PANELS` in `src/config/panels.ts`.
...
**Free.** The panel is no longer registered with `premium: 'locked'`, and its data comes from the public bootstrap snapshot (`wsbTickers`) rather than a premium-only RPC path.
