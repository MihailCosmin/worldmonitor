...
- **Sign-in required** — a neutral empty state when the reader is signed out. The brief is still tied to the signed-in WorldMonitor account that owns it.
...
The panel calls `GET /api/latest-brief` (Clerk JWT required). The response is either the ready payload (`issueDate`, `dateLong`, `greeting`, `threadCount`, `magazineUrl`) or a `{ status: "composing", issueDate }` stub. The signed `magazineUrl` is generated server-side — the HMAC signing secret never lives in the client bundle.
...
Latest Brief is available to any signed-in WorldMonitor user. The panel is no longer registered with `premium: 'locked'`, and `api/latest-brief.ts` now checks only for a valid Clerk JWT before returning the caller's latest composed brief preview. Signed-out visitors still see the neutral sign-in state because the brief remains tied to the owning account.
