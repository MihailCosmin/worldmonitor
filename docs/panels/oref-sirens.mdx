...
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** variant only (`priority: 2`), free for all users on both web and desktop — no `premium` field. Not present in the tech, finance, commodity, or happy variants. Source: `'oref-sirens'` in `FULL_PANELS` of `src/config/panels.ts`.
