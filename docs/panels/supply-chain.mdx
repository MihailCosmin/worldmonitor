...
The **Supply Chain** panel (internal id `supply-chain`) is the main entry point for everything supply-chain related: a composite shipping-stress score, carrier and ETF rows with price movement, a minerals concentration view, and — most importantly — the **[Scenario Engine](/scenario-engine)** trigger buttons that let users activate pre-built disruption scenarios across the map and the panel state.
...
- **Scenario Engine trigger cards** — per-scenario trigger buttons (`sc-scenario-trigger` / `sc-scenario-btn`) that dispatch a template id to the scenario worker. Scope-all scenario runs use the v1 seeded reporter set (`US`, `CN`, `RU`, `IR`, `IN`, `TW`). See [Scenario Engine](/scenario-engine) for the full workflow.
...
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** (priority 1), **finance** (priority 1), and **commodity** (priority 1) variants, free in all three on both web and desktop. Not present in the tech or happy variants. Source: `'supply-chain'` entries in `FULL_PANELS`, `FINANCE_PANELS`, `COMMODITY_PANELS`.
...
| Country exposure | `GET /api/supply-chain/v1/get-country-chokepoint-index` | Country + HS2 exposure scores used by Country Brief and route-risk surfaces. |
| Bypass corridors | `GET /api/supply-chain/v1/get-bypass-options` | Ranked maritime and land bypass options for a chokepoint / cargo type. |
| Country cost shock | `GET /api/supply-chain/v1/get-country-cost-shock` | Cost-shock and war-risk estimates for a country + chokepoint + HS2 slice. Free for all users. |
| Country import basket | `GET /api/supply-chain/v1/get-country-products` | Seeded bilateral-HS4 import basket for country drill-downs. |
...
| Multi-sector shock | `GET /api/supply-chain/v1/get-multi-sector-cost-shock` | Sector-level cost-shock estimates over a closure window. |
| Sector dependency | `GET /api/supply-chain/v1/get-sector-dependency` | Dependency flags, primary exporter share, and primary chokepoint exposure. |
| Route Explorer lane | `GET /api/supply-chain/v1/get-route-explorer-lane` | Public primary route geometry, chokepoint exposures, bypass geometry, war-risk tier, and disruption score. |
| Route Explorer impact | `GET /api/supply-chain/v1/get-route-impact` | Public country-impact metrics for selected lane + HS2 commodity. |
...
| Surface | Method + route | Feeds |
|---|---|---|
| Scenario activation | `POST /api/scenario/v1/run-scenario` | Scenario Engine trigger buttons dispatch a template id and parameters. |
...
**Free**, including every RPC listed above, on web and desktop across the full, finance, and commodity variants. Scenario Engine activation is available to free users on web and desktop.
