...
- **Cmd+K**: type *telegram*.
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** variant only (`priority: 2`), free for all users on both web and desktop — no `premium` field. Not present in the tech, finance, commodity, or happy variants. Source: `'telegram-intel'` in `FULL_PANELS` of `src/config/panels.ts`.
...
**Free**, on both web and desktop. No `premium` field on the panel's registration.
