...
**Free** — no `premium` field on the panel's registration, for anonymous and signed-in readers alike, on both web and desktop. One exception: `POST /api/forecast/v1/trigger-simulation` (an on-demand simulation run outside the cached cron output) remains Pro-gated (`PREMIUM_RPC_PATHS`); its read-side companion `get-simulation-outcome` is public like the rest of the panel.

## API reference

- [Forecast service](/api/ForecastService.openapi.yaml) — covers `get-forecasts`, `get-forecast-scorecard`, `get-simulation-outcome`, `get-simulation-package`, and `trigger-simulation`.
