...
Panel id is `stock-analysis`; canonical component is `src/components/StockAnalysisPanel.ts`. Title "Stock Analysis" per the panel's own `super({...})` call.

## How you reach it

- **Cmd+K**: type *stock* or the ticker symbol.
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** and **finance** variants — free in both. Not present in the tech, commodity, or happy variants. Source: `'stock-analysis'` entries in `FULL_PANELS` and `FINANCE_PANELS` at `src/config/panels.ts:49` and `:422`.

## Data sources

All fields come from generated `MarketServiceClient` routes. `src/services/stock-analysis.ts` calls them via `premiumFetch(..., { forcePremium: true })`, which attaches a Clerk bearer or `WORLDMONITOR_API_KEY` when one is available — this is a boot-window auth-race workaround for signed-in callers (see the file-level comment in `src/services/premium-fetch.ts`), not a plan gate. Anonymous callers fall straight through to the normal unauthenticated path and are served identically; none of `analyze-stock`, `get-stock-analysis-history`, or `get-insider-transactions` is in `PREMIUM_RPC_PATHS`.
...
**Free.** No `premium` field on either variant registration, and none of the panel's RPCs are in `PREMIUM_RPC_PATHS`.
...
- Related: [Backtesting](/panels/stock-backtest) — the backtest side of the same dataset.
