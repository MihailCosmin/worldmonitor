...
Above the cards sits a **Framework selector** — the same `FrameworkSelector` used by other LLM panels — which lets users re-run the analysis against an alternative analytical framework.
...
- **Availability by variant**: registered and enabled by default in the **full/geopolitical** variant only. Not present in the tech, finance, commodity, or happy variants.
...
Public panel on the web app. The panel is enabled in the full variant without a Pro/API-key gate, and the framework selector remains available to free users.
