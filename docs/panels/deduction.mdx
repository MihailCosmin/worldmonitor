...
- **Availability by variant**: registered in the **full/geopolitical** variant only, and **disabled by default** (`enabled: false` at `src/config/panels.ts:111`; no `premium` field). Any user can opt into it. Not present in the tech, finance, commodity, or happy variants.
...
**Free, opt-in, and signed-in only.** `enabled: false` in `src/config/panels.ts:111` (no `premium` field) — the panel itself has no plan gate. The one requirement is identity: `canUseDeductionAnalysis()` in `src/components/DeductionPanel.ts` requires `hasPremiumAccess() || getCurrentClerkUser() !== null`, so any signed-in user (free or paid) can submit — anonymous callers cannot, because the gateway needs a real Clerk bearer to bind direct-LLM spend to a user rather than an anonymous session.
