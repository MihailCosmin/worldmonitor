...
来自 `@/services/trade` 的六种响应形状，均由 `TradeService` 中生成的 sebuf REST RPC 支持。六个标签页现在都使用公开的生成客户端；海关收入仍会在回退到 RPC 之前检查 bootstrap 缓存键 `customsRevenue`。

| 标签页 | 方法 + 路由 | 支撑内容 |
|---|---|---|
| `restrictions` | `GET /api/trade/v1/get-trade-restrictions` | WTO 限制列表，含国家过滤器与上游不可用状态。 |
| `tariffs` | `GET /api/trade/v1/get-tariff-trends` | 申报国与伙伴国之间的关税率时间序列。 |
| `flows` | `GET /api/trade/v1/get-trade-flows` | 双边商品贸易流行与同比上下文。 |
...
| `comtrade` | `GET /api/trade/v1/list-comtrade-flows` | UN Comtrade 战略商品流搜索，含异常标志。 |
...
**免费。** 变体注册中无 `premium` 标志，且六个标签页依赖的 RPC 现已全部向免费用户开放。

## API 参考

- [Trade service](/api/TradeService.openapi.yaml) — 六个标签页支持 RPC 的完整 schema 与查询参数。
