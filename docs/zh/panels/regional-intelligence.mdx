...
- **按变体的可用性**：仅在 **full/geopolitical** 变体中注册，且**默认禁用**（`src/config/panels.ts` 中的 `enabled: false`）。免费用户可手动启用。在 tech、finance、commodity 或 happy 变体中不存在。
...
公开的 Web 面板。它在 full 变体中仍然是手动启用项，但加载区域看板及其扩充板块不再需要 Pro 或 API key。

## API 参考

- [Intelligence service](/api/IntelligenceService.openapi.yaml) — `GetRegionalSnapshot` 及相关 RPC。
