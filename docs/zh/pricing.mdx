...
| **Pro Business** | $49.99/月 · $449.99/年 | 涵盖 Pro 的全部内容，并可用于客户项目、内部工具与报告 — 数据导出、250 次 MCP 调用/天、优先支持 |
...
- **Free** — 56 个地图图层、500+ 精选数据源（背后有 530+ 个受监测的上游主机）、国家简报、热点、不稳定性评分、咽喉要道、级联分析、突发警报和监视列表，仪表盘标签页数量不受限制。仪表盘的刷新频率通常为 5–15 分钟。
- **Pro** — 涵盖 Free 的全部内容，外加跨 30+ 实时服务并带引用的 WM Analyst 聊天、Scenario Engine、Route Explorer（路线探索器）、个人 AI 摘要（每日 / 每日两次 / 每周，通过 Slack、Discord、Telegram、邮件、webhook 发送）、自定义小部件构建器，以及 MCP 访问（一个密钥下 59 个工具，50 次调用/天）。个人许可。
