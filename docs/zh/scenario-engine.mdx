...
UI 是状态驱动而非模态 — 激活场景在每个地图渲染器（deck.gl、globe、SVG 回退）上设置 `scenarioState`，使咽喉要道颜色和国家分级统计图反映中断，直到你停用。这由 `src/components/MapContainer.ts` 中的 `MapContainer.activateScenario` 协调。

API 侧的速率限制 — 10 个作业/分钟/IP，一旦待处理队列已超过 100 个作业即施加队列背压 — 记录在 [Scenarios API](/zh/api-scenarios#run-a-scenario) 中。
...
对于脚本化使用，请参见 [`POST /api/scenario/v1/run-scenario`](/zh/api-scenarios#run-a-scenario) — 入队，然后轮询 `GET /api/scenario/v1/get-scenario-status` 直到响应有终止状态（成功为 `"done"`，错误为 `"failed"`）。非终止状态为 `"pending"`（排队）和 `"processing"`（worker 已启动）；两者都可能持续数秒。请参见[状态生命周期表](/zh/api-scenarios#poll-job-status)了解完整合约。
...
- [Supply Chain](/api/SupplyChainService.openapi.yaml) — 支持 Supply Chain 面板的更广泛服务。
