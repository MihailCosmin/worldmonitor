---
title: "Stock-News Search Layer"
description: "The targeted stock-news search layer that powers Stock Analysis — how ticker queries are dispatched, ranked, and cached across upstream providers. Free for all users."
---
This document covers the **extra** targeted stock-news search layer used by Stock Analysis. Stock Analysis itself is free for all users — see [Access](/premium-finance#access).

It is separate from the core stock-analysis architecture on purpose.

The core stock-analysis engine can still function without this layer. The search layer exists to improve stock-news discovery quality for targeted ticker analysis, especially where feed-only coverage is weak.

See the core system document in [Stock Analysis & Backtesting Architecture](/premium-finance).
...
`Bocha` was intentionally not added because the current stock-analysis direction is not China-focused.
...
This avoids repeated provider calls when multiple users request the same stock analysis.
...
- stock analysis does not hard-depend on paid search providers
...
- the source of truth for stock-analysis persistence

That separation matters because it keeps the stock-analysis feature understandable:
...
Right now it is an internal backend helper used by stock analysis. That is deliberate:
