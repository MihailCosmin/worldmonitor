...
All three dashboard surfaces are free to view. `get-resilience-score` and `get-resilience-ranking` are free for all users — neither is in `PREMIUM_RPC_PATHS` or `ENDPOINT_ENTITLEMENTS` — and the runtime manifest is public at `/api/resilience/v1/get-runtime-manifest`; see [Resilience service](/api/ResilienceService.openapi.yaml) for the HTTP contract.
