...
Route Explorer is public in the web app. Free users can run the full Current / Alternatives / Land / Impact workflow and see the same lane and impact surfaces without a Pro-only upgrade gate.

## Data behind Route Explorer

Route Explorer calls generated sebuf REST RPCs from `SupplyChainService`. The route-intelligence vendor path is wrapped server-side by `get-route-explorer-lane`; browser code should use the generated REST routes below.
