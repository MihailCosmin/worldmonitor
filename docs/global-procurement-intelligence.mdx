...
Global Procurement is now a **public feature**. Free users can search, filter, and paginate the same canonical tender feed without a subscription gate. The panel still keeps its own response cache local to the browser session, and recent awards remain part of the free Economic panel behavior.

Users can search titles and descriptions, filter by buyer, two-letter country code, and source, choose a supported sort, and page through the complete result set. The panel never reads Upstash directly: it calls the paginated RPC, which reads the canonical seeded snapshot.
...
The MCP tool uses the same public HTTP route and is not part of bootstrap. An unfiltered tool call preserves all open opportunities; `min_automation_score` remains opt-in. See the [MCP tool reference](/mcp-tools-reference#get_procurement_opportunities) for the complete parameter contract.
