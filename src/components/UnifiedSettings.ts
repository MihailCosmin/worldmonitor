...
import { getAuthState, signIn, subscribeAuthState } from '@/services/auth-state';
...
export class UnifiedSettings {
  ...
  private render(loadAccountData = true): void {
    ...
    if (loadAccountData) {
      ...
      // No hasFeature('apiAccess') check: this fork removed the apiAccess
      // paywall from Settings > API Keys (feature-ungate) — any signed-in
      // user can manage their own keys, sign-in itself is the only gate.
      if (this.activeTab === 'api-keys' && getAuthState().user) {
        void this.loadApiKeys();
      }
